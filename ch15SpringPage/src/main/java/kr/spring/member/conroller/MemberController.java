...
public class MemberController {
	...
	public String getProfile(HttpSession session,
		...
		if(user == null) { //로그인이 되지 않은 경우
			getBasicProfileIamge(request, model);
		}else {
			MemberVO memberVO = memberService.selectMember(user.getMem_num());
	...
	public void viewProfile (MemberVO memberVO,
		...
		if(memberVO == null || memberVO.getPhoto_name() == null) {
			//DB에 저장된 프로필 이미지가 없기 떄문에 기본 이미지 호출
			getBasicProfileIamge(request, model);
		}else {
			//업로드한 프로필 이미지 읽기
	...
	public void getBasicProfileIamge(HttpServletRequest request,
									Model model) {
	...
	public String getCaptcha(Model model, HttpSession session) {
		String clientId = "PPJDiC3BNDi3uGhs0FV3";
		String clientSecret = "bVFWG1tBcn";
		...
		try {
			//https://openapi.naver.com/v1/captcha/nkey 호출로 받은 key 값
